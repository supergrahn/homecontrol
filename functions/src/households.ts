...
export const deleteHouseholdRecursive = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Sign in required');
  const hid: string = data?.householdId;
  if (!hid) throw new functions.https.HttpsError('invalid-argument', 'householdId is required');
  ...
  if (!memberSnap.exists || memberSnap.get('role') !== 'admin') {
    throw new functions.https.HttpsError('permission-denied', 'Only admins can delete a household');
  }
  ...
  membersSnap.docs.forEach((m: admin.firestore.QueryDocumentSnapshot) => {
    ...
    batch.update(userRef, { householdIds: admin.firestore.FieldValue.arrayRemove(hid) });
  });
  await batch.commit();
