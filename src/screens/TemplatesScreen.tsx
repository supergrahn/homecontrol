...
export default function TemplatesScreen() {
  ...
  const addTemplate = async (name: string) => {
    ...
    const taskId = await createTask(householdId, { title: name, type: 'checklist', createdBy });
    for (const label of templates[name]) {
      await addChecklistItem(householdId, taskId, label);
    }
    Alert.alert(t('templateCreated'), t('templateAdded', { name, count: templates[name].length }));
  };

  return (
    <View style={{ flex:1, padding:16 }}>
    <Text style={{ fontSize:22, fontWeight:'700', marginBottom:12 }}>{t('templates')}</Text>
  {Object.keys(templates).map(k => (
        <View key={k} style={{ marginBottom:16 }}>
          <Text style={{ fontWeight:'600', marginBottom:6 }}>{k}</Text>
  <Button title={t('add')} onPress={() => addTemplate(k)} disabled={!householdId} />
        </View>
      ))}
    </View>
  );
